...
Public Const def_VoteForStadmisURL = "http://www.Planet-Source-Code.com/vb/scripts/ShowCode.asp?txtCodeId=74283&lngWid=1"
...
Public vEditRecordID$, vSelTbl$, vStatus$
...
CreateButtons:
        ...
        Else
            ...
            If vAutoCloseDuration > &H0 Then .TxtPrompt.Text = VBA.Replace(.TxtPrompt.Text, "£", vAutoCloseDuration)
        ...
        '---------------------------------------------------------------
'        If MyDefButton <= .ShpBttnButton.UBound Then .ShpBttnButton(MyDefButton).Default = True
        
        .Tag = VBA.IIf(vUseDefault, VBA.IIf(((nButtons - (MyDefButton / 256)) - &H1) < &H0, nButtons - &H1, ((nButtons - (MyDefButton / 256)) - &H1)), -&H1)
        
        '.Tag = VBA.IIf(vUseDefault, VBA.IIf(MyDefButton >= &H0, nButtons - ((MyDefButton / 256) - &H1), nButtons - ((256 * nButtons) / 256)), -&H1)
        '---------------------------------------------------------------
...
RequestPwd:
            ...
            VB.Clipboard.Clear: VB.Clipboard.SetText SmartEncrypt("Stadmis..", False)
            
            Load Frm_DataEntry
'            Frm_DataEntry.IsPassword = True
            Frm_DataEntry.OptEncryption(&H0).Visible = True
            Frm_DataEntry.mTrials = 3 - Trials
            ...
            If vBuffer(&H0) = "Cancelled" Or vBuffer(&H0) = VBA.vbNullString Then vMsgBox "The Application will automatically terminate after £ seconds!!", vbExclamation, App.Title & " : Unspecified Unlock Password", , , , &H8: GoTo QuitApp
            ...
            If Not CheckDBPwdValidity(vAdoCNN.Properties("Data Source"), dbPwd) Then
                ...
                If Trials < &H3 Then
                    ...
                    If vMsgBox("The entered Password is incorrect. Retry?", vbQuestion + vbYesNo + vbDefaultButton1, App.Title & " : Invalid Db Password") = vbYes Then GoTo RequestPwd
                ...
                Else 'If the specified max no of incorrect entries has been reached then
                    ...
                    GoTo QuitApp 'Quit this Application
...
On Local Error GoTo Handle_FillListView_Error
    ...
    With vRs 'Execute a series of statements on vRs recordset
        ...
        ShowPleaseWait = (.RecordCount > 500)
                        ...
                        If Not VBA.IsNull(nRs(vFldSection(&H0))) Then
                            ...
                            ElseIf VBA.InStr(vFldSection(&H1), "<>") <> &H0 Then
                                ...
                                Else
                                    ...
                                    'If the condition is true then change its color to the specified one
                                    If VBA.UCase$(nRs(vArrayListTmp(&H0))) <> VBA.UCase$(VBA.Replace(VBA.Replace(vFldSection(&H1), "<>", VBA.vbNullString), "''", VBA.vbNullString)) Then Lst.ForeColor = vArrayListTmp1(vIndex(&H0)): Exit For
...
NextCondition:
                    ...
                    Lst.ToolTipText = VBA.IIf(VBA.CLng(vBuffer(&H0)) = VBA.CLng(vArrayListTmp1(vIndex(&H0))), "[" & vArrayListTmp(&H0) & "] is """ & nRs(vArrayListTmp(&H0)) & """", VBA.vbNullString)
...
Public Function IsWebConnected(Optional ByRef ConnType As String) As Boolean
    ...
    ConnType = VBA.vbNullString
    WebTest = InternetGetConnectedState(dwflags, 0&)
    
    Select Case WebTest
        
        Case dwflags And CONNECT_LAN: ConnType = "a LAN"
        Case dwflags And CONNECT_MODEM: ConnType = "a Modem"
        Case dwflags And CONNECT_PROXY: ConnType = "a Proxy"
        Case dwflags And CONNECT_OFFLINE: ConnType = "Offline"
        Case dwflags And CONNECT_CONFIGURED: ConnType = "Configured"
        Case Else: ConnType = "Remote"
...
VoteProject:
        ...
        Else 'If the Computer is not connected to the Internet then...
            ...
            'Warn User
            If vMsgBox("Your Computer is not connected to the internet. Retry?", vbQuestion + vbRetryCancel + vbDefaultButton1, App.Title & " : No Internet Connection", vFrm) = vbRetry Then GoTo VoteProject
...
Handle_OpenFile_Error:
    ...
    'Warn User
    vMsgBox Err.Description, vbExclamation, App.Title & " : Error Accessing " & VBA.IIf(vIsURL, "URL", "File") & " - " & Err.Number, vFrm
    ...
    For Each LvLst In vLv.ListItems
        ...
        Select Case LvColType
            ...
            Case LvDateTime: 'DATE/TIME then...
                ...
                Lst.Text = VBA.FormatDateTime(VBA.Format$(Lst.Text, "dd MMM yyyy"), vbShortDate)
