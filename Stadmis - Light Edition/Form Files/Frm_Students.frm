...
Begin VB.Form Frm_Students 
   ...
   Begin prjXTab.XTab XTab 
      ...
      Begin VB.Frame Fra_Student 
         ...
         Top             =   360
         Width           =   6015
         Begin VB.ComboBox cboStudentStatus 
            BackColor       =   &H00C0FFFF&
            BeginProperty Font 
               Name            =   "Tahoma"
               Size            =   8.25
         ...
         Begin VB.ComboBox cboGender 
            ...
            ForeColor       =   &H00C00000&
            Height          =   315
            ItemData        =   "Frm_Students.frx":36E5
            Left            =   3000
            List            =   "Frm_Students.frx":36EF
            Style           =   2  'Dropdown List
            TabIndex        =   9
            Top             =   1680
            Width           =   855
         ...
         Begin VB.ComboBox cboMaritalStatus 
            ...
            ForeColor       =   &H00C00000&
            Height          =   315
            ItemData        =   "Frm_Students.frx":3701
            Left            =   3960
            List            =   "Frm_Students.frx":370B
            Style           =   2  'Dropdown List
            TabIndex        =   25
            Top             =   3480
            Width           =   1935
         ...
         Begin Stadmis.ShapeButton ShpBttnAttachPhoto 
            ...
            EndProperty
         End
         Begin VB.Label lblStudentStatus 
            AutoSize        =   -1  'True
            BackStyle       =   0  'Transparent
...
On Local Error GoTo Handle_DisplayRecord_Error
    ...
    With vRs 'Execute a series of statements on vRs recordset
        ...
        If Not (.BOF And .EOF) Then
            ...
            If Not VBA.IsNull(![Student Status]) Then cboStudentStatus.Text = ![Student Status]
...
Private Sub cboStudentStatus_Click()
On Local Error GoTo Handle_cboStudentStatus_Click_Error
    
    If myRecDisplayON Then cboStudentStatus.Tag = cboStudentStatus.ListIndex: Exit Sub
    
    If Not ValidUserAccess(Me, iSetNo, &H4) Then Exit Sub
    
    If cboStudentStatus.ListIndex <> &H3 Then cboStudentStatus.Tag = cboStudentStatus.ListIndex: Exit Sub
    ...
    If vMsgBox("Ticking this option will disable the Record and will not be available in other Modules. Proceed?", vbQuestion + vbYesNo + vbDefaultButton2, App.Title & " : Confirmation") = vbNo Then myRecDisplayON = True: cboStudentStatus.ListIndex = VBA.Val(cboStudentStatus.Tag): myRecDisplayON = False
    
Exit_cboStudentStatus_Click:
    
    cboStudentStatus.Tag = cboStudentStatus.ListIndex
...
Handle_cboStudentStatus_Click_Error:
    ...
    'Resume execution at the specified Label
    Resume Exit_cboStudentStatus_Click
...
Private Sub Fra_Photo_Click(Index As Integer)
    ...
    Call PhotoClicked(ImgDBPhoto(Index), ImgVirtualPhoto, Fra_Photo(Index), VBA.IIf(Index = &H1, True, Not (Fra_Student(&H0).Enabled)), VBA.IIf(Not Fra_Student(&H0).Enabled, "2", ""))
End Sub
...
On Local Error GoTo Handle_MnuSave_Click_Error
    ...
    With vRs 'Execute a series of statements on vRs recordset
        ...
        ![Student Status] = cboStudentStatus.Text
    ...
    vRsTmp.Open "SELECT * FROM [Tbl_StudentDormitories] WHERE YEAR([Date Assigned]) = " & VBA.Year(dtAdmissionDate.Value) & " AND [Student ID] = " & VBA.Val(txtSurname.Tag), vAdoCNN, adOpenKeyset, adLockPessimistic
    If cmdClearDormitory.Tag <> VBA.vbNullString Then vRsTmp.Filter = "[Student Dormitory ID] = " & VBA.Val(cmdClearDormitory.Tag) Else vRsTmp.Filter = "[Dormitory ID] = " & VBA.Val(txtDormitoryName.Tag)
    
    If Not (vRsTmp.BOF And vRsTmp.EOF) Then
        
        'If no Dormitory has been selected then...
        If txtDormitoryName.Tag = VBA.vbNullString Then
            
            If vMsgBox("The Student had been assigned a dormitory. Do you want to cancel the allocation?", vbQuestion + vbYesNo + vbDefaultButton2, App.Title & " : Confirming Dormitory Allocation", Me) = vbYes Then
                vRsTmp.Delete
            End If 'Close respective IF..THEN block statement
    ...
    Else
        If txtDormitoryName.Tag = VBA.vbNullString Then GoTo SaveGuardians Else vRsTmp.AddNew
    
    End If 'Close respective IF..THEN block statement
    
    vRsTmp![Date Assigned] = VBA.FormatDateTime(dtAdmissionDate.Value, vbShortDate)
    vRsTmp![Student ID] = VBA.Val(txtSurname.Tag)
    vRsTmp![Dormitory ID] = VBA.Val(txtDormitoryName.Tag)
    vRsTmp![Date Last Modified] = VBA.Now
    vRsTmp![User ID] = VBA.IIf(VirtualUser.User_Name <> VBA.vbNullString, VirtualUser.User_ID, User.User_ID)
